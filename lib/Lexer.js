...
var numericRegex = /^-?(?:(?:[0-9]*\.[0-9]+)|[0-9]+)$/,
  // varIdentRegex = /^[a-z_\$][a-zA-Z0-9_\$]*$/,
  // typeIdentRegex = /^[A-Z][a-zA-Z0-9_\$]*$/,
...
function Token(grammar, element) {
  this.type = 'literal';
  this.value = element;
  this.raw = element;
  if (element[0] == '"' || element[0] == "'")
    this.value = unquote(element);
  else if (numericRegex.test(element))
    this.value = parseFloat(element);
  else if (element === 'true' || element === 'false')
    this.value = element === 'true';
  else if (grammar[element])
    this.type = grammar[element].type;
...
Lexer.prototype._createToken = function(element) {
  return new Token(this._grammar, element);
};
...
Lexer.prototype._getSplitRegex = function() {
  if (!this._splitRegex) {
    ...
    var elemArray = Object.keys(this._grammar).sort(function(a ,b) {
