...
Evaluator.prototype.evalSub = function(ast, subProc, obj) {
	subProcessor = new subProcessors[subProc](this, ast, obj);
...
Evaluator.prototype._subProc = function(ast, subProc) {
	var subject = this.evalLazy(ast.subject),
		evalSub = this.evalSub.bind(this, ast, subProc);
	if (subject instanceof Relay)
		return evalSub(subject);
	return utils.partialConsume(subject).then(evalSub);
};
...
MetaEvaluator.prototype.eval = function(obj) {
	...
	if (obj instanceof Relay)
		result = this.evalRelay(obj);
	else if (Array.isArray(obj))
		result = this.evalRelay(Relay.from(obj, true));
	else
		result = this.evalOne(obj);
	if (this._consume)
		return utils.consume(result);
	return result instanceof Relay ? result : Relay.from(result, true);
};

MetaEvaluator.prototype.evalRelay = function(relay) {
	return relay.map(this.evalOne.bind(this));
};

MetaEvaluator.prototype.evalArray = function(arr) {
	return this.evalRelay(Relay.from(arr, true));
};

MetaEvaluator.prototype.evalObject = function(context) {
	return this._consume ? utils.consume(this.evalOne(context)) : this.evalRelay(Relay.from([context], true));
};
