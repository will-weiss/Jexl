...
function SubProcessor(evaluator, ast, obj) {
	...
	this.ast = ast;
	this.relay = obj instanceof Relay ? obj : Relay.from(obj, true);
...
/**
 * When the object being collected is an Relay, the result is an Relay.
 */
Collector.prototype.process = function() {
	return this.relay.map(this.subOp.bind(this))
	.filter(function(res) {
		return res !== omit;
	});
};
...
/**
 * Applies processing logic for when the supplied object is an Relay. Keys
...
Finder.prototype.process = function() {
	var self = this;
	return this.relay.find(function(val, key) {
...
Reducer.prototype.process = function() {
	...
	return this.relay.reduce(function(accumulator, val, key) {
		context['$'] = accumulator;
		return self.subOp(val, key).then(function(res) {
