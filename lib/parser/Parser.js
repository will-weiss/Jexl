 ...
 * @constructor
 */
function Parser(grammar, prefix, stopMap, startingState) {
  this._grammar = grammar;
  this._state = startingState || 'expectOperand';
  this._tree = null;
  this._exprStr = prefix || '';
  this._relative = !!stopMap;
...
Parser.prototype.addToken = function(token) {
  ...
  if (state.subHandler) {
    ...
    var handleFunc = typeOpts.handler || handlers[token.type];
    var toProcess = typeOpts.parseToken ? this.parseToken(token) : token;
    if (handleFunc)
      handleFunc.call(this, toProcess);
    if (typeOpts.toState)
...
Parser.prototype.parseToken = function(token) {
  var p = new Parser(this._grammar);
  p.addToken(token);
  return p.complete();
}
