...
exports.states = {
  programStart: {
    subHandler: h.programStart
  },
  expectOperand: {
    tokenTypes: {
      literal: {toState: 'expectBinOp'},
      variableIdentifier: {toState: 'variableIdentifier'},
      typeIdentifier: {toState: 'typeIdentifier'},
      unaryOp: {},
  ...
  expectObjKey: {
    tokenTypes: {
      variableIdentifier: {toState: 'expectKeyValSep', handler: h.objKey},
  ...
  variableIdentifier: {
    tokenTypes: {
      binaryOp: {toState: 'expectOperand'},
      colon: {toState: 'expectIdent', handler: h.variableDefinition},
      dot: {toState: 'apply'},
      equals: {toState: 'expectOperand', handler: h.variableDefinition},
      literal: {toState: 'variableIdentifier', handler: h.callExpression, parseToken: true},
      openBracket: {toState: 'filter'},
      openParen: {toState: 'subExpression', handler: h.callExpression},
      union: {toState: 'expectOperand'},
      question: {toState: 'ternaryMid', handler: h.ternaryStart},
      variableIdentifier: {toState: 'variableIdentifier', handler: h.callExpression, parseToken: true}
    },
    completable: true
  },
  expectIdent: {
    tokenTypes: {
      variableIdentifier: {toState: 'maybeArrow', handler: h.typeDeclaration}
    }
  },
  maybeArrow: {
    tokenTypes: {
      arrow: {toState: 'expectIdent', handler: h.arrow},
      equals: {toState: 'expectOperand'}
    },
    completable: true
  },
  typeDeclaration: {
    subHandler: h.typeDeclaration,
    endStates: {
      arrow: 'typeDeclaration',
      equals: 'expectOperand'
    },
    completable: true
  },
  typeIdentifier: {
    tokenTypes: {
      equals: {toState: 'expectOperand', handler: h.typeDefinition}
    }
  },
  apply: {
    tokenTypes: {
      'variableIdentifier': {toState: 'variableIdentifier', handler: h.apply, parseToken: true}
    }
  },
  filter: {
    subHandler: h.filter,
    endStates: {
      closeBracket: 'variableIdentifier'
    }
  },
  subExpression: {
