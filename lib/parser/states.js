...
exports.states = {
  ...
  identifier: {
    tokenTypes: {
      binaryOp: {toState: 'expectOperand'},
      colon: {toState: 'expectIdent', handler: h.definition},
      dot: {toState: 'apply'},
      equals: {toState: 'expectOperand', handler: h.definition},
      literal: {toState: 'identifier', handler: h.callExpression, parseToken: true},
      openBracket: {toState: 'filter'},
      openCurl: {toState: 'expectRecordKey', handler: h.recordStart},
      openParen: {toState: 'subExpression', handler: h.callExpression},
      union: {toState: 'expectOperand'},
      identifier: {toState: 'identifier', handler: h.callExpression, parseToken: true}
    },
    completable: true
  },
  expectEquals: {
    tokenTypes: {
      equals: {toState: 'recordVal'}
    }
  },
  expectRecordKey: {
    tokenTypes: {
      identifier: {toState: 'expectEquals', handler: h.objKey},
      closeCurl: {toState: 'expectBinOp'}
    }
  },
  recordVal: {
    subHandler: h.objVal,
    endStates: {
      comma: 'expectRecordKey',
      closeCurl: 'expectBinOp'
    }
  },
  expectIdent: {
    tokenTypes: {
      identifier: {toState: 'maybeArrow', handler: h.typeDeclaration}
    }
  },
  maybeArrow: {
    tokenTypes: {
  ...
  apply: {
    tokenTypes: {
      'identifier': {toState: 'identifier', handler: h.apply, parseToken: true}
    }
  },
  typeDeclaration: {
